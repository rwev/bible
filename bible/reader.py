...
class Reader():
    ...
    def get_translations(self):
        return [splitext(basename(f))[0] for f in
                glob('{0}/*.xml'.format(TRANSLATIONS_DIR))]
    ...
    def get_verses_elements(self, book_str, chapter_str):
        return self._current_root[1].find('b[@n=\'{0}\']'.format(book_str)).find('c[@n=\'{0}\']'.format(chapter_str)).findall('v')
